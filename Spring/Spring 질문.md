...
- Spring의 프로토 타입 스코프 빈에 대해 설명해주세요
```
일반적으로 Bean 은 싱글톤 scope 을 갖는다. 오직 하나의 인스턴스만 생성해서 사용하게 되는데 Bean 의 scope 설정은 @Scope 어노테이션으로 설정할 수 있다.
이때 @Scope("prototype") 으로 설정을 하면 모두 다른 인스턴스가 생성이 되는데, 싱글톤과 달리 IoC 에서 빈을 받아올 때 매번 인스턴스를 새로 생성한다. 즉, 프로토타입 빈은 생성과 의존관계 주입까지만 스프링 컨테이너가 관여하고 그 이후는 관리하지 않는 매우 짧은 범위의 스코프이다.

* 싱글톤 빈과 같이 사용 시 문제
만약 싱글톤 빈을 생성할 때 프로토타입 스코프 빈도 같이 사용하게 되는 로직이 있다면, 싱글톤 빈과 함께 사용되기 때문에 프로토타입 스코프 빈이어도 인스턴스가 새로 생성되지 않고 하나의 인스턴스가 계속 유지된다.
```
