...
```
트랜잭션이란, 일관된 상태로 유지하는 단일 작업 단위이다.

Atomicity 원자성
모든 작업이 반영되거나 롤백이 되어야 한다. 이 속성은 중간에 실패하는 경우 데이터 손실 및 손상이 발생하지 않도록 방지한다.

Consistency 일관성
트랜잭션이 미리 정의되고 예측 가능한 방식으로만 테이블을 변경하도록 한다.
데이터의 손상이나 오류로 인해 테이블 무결성에 의도하지 않은 결과가 발생하지 않도록 한다.

Isolation 격리
두 개의 트랜잭션이 실행되고 있을 때, 격리하면 동시 트랜잭션이 서로 간섭하거나 영향을 미치지 않는다. 각 요청은 실제로 동시에 발생하더라도 하나씩 발생하는 것처럼 발생할 수 있다.

Durability 영구성
한 번 커밋된 트랜잭션의 내용은 영원히 적용되는 특성이다.
```
