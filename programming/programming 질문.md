...
주로 사용자의 인증 또는 인가 정보를 서버와 클라이언트 간에 안전하게 주고 받기 위해서 사용된다. 보통 HTTP 헤더에 Authorization, Bearer 토큰 값으로 넣으며 클라이언트에서 서버로 전송되면 이 값을 서버에서 검증하여 로직 처리를 하게 된다.
JWT 토큰은 헤더, 페이로드, 서명 이렇게 구성되어 있는데 헤더는 토큰 유형과 서명 알고리즘이 명시되고, 페이로드에는 인증/인가 정보인 claim 이 담긴다. 마지막 서명에는 헤더와 페이로드가 비밀키로 서명되어 저장된다.
```
